...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <!-- WRAPPER -->
    <div class="tw-p-6 tw-bg-gray-50 tw-min-h-screen tw-text-gray-800 tw-space-y-6">
      <!-- CARD DETAIL -->
      <div
      ...
      >
        ...
        <div class="tw-flex tw-justify-center md:tw-justify-start">
          <img
            :src="transaction?.order?.product_image || '/img/no-image.png'"
            alt="Produk"
        ...
        <div class="tw-flex-1 tw-space-y-3 tw-relative">
          ...
          <!-- NAMA PRODUK -->
          <h1 class="tw-text-2xl tw-font-bold tw-text-blue-900 tw-leading-snug">
            {{ transaction?.order?.product_name ?? "Nama Produk Tidak Tersedia" }}
          </h1>

          <!-- HARGA -->
          <p class="tw-text-orange-600 tw-font-bold tw-text-lg">
            Rp {{ Number(transaction.selling_price_total).toLocaleString("id-ID") }}
          </p>
          ...
          <div class="tw-space-y-1.5 tw-text-sm tw-mt-2">
            ...
            <template v-if="transaction.order?.payment_method === 'Murabahah'">
              <p v-if="transaction.tenor">
                <strong>Tenor:</strong> {{ transaction.tenor }} bulan
              </p>
              <p v-if="tenorPeriod">
