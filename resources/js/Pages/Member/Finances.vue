...
const allHistory = computed(() => {
    const diterima =
        props.historytopup.historyditerima?.map((item) => ({
            nominal: item.amount,
    ...
    const ditolak =
        props.historytopup.historyditolak?.map((item) => ({
            nominal: item.amount,
            tanggal: new Date(item.transaction_date),
            status: "Ditolak",
            admin_user_id: item.admin_user_id,
    ...
    const pending =
        props.historytopup.historypending?.map((item) => ({
            nominal: item.amount,
            tanggal: new Date(item.transaction_date),
            status: "Pending",
            admin_user_id: item.admin_user_id,
    ...
    // Spread Operator //
    return [...pending, ...diterima, ...ditolak].sort(
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <!-- Total Saldo -->
        <div
            class="tw-bg-gray-50 tw-p-5 tw-flex tw-flex-col tw-items-center tw-gap-5"
        >
            <div
                class="tw-bg-gradient-to-r tw-from-orange-500 tw-to-orange-400 tw-rounded-2xl tw-w-full tw-text-white tw-p-6 tw-shadow-lg"
            >
                <h3 class="tw-text-lg tw-font-medium">Total Saldo</h3>
                <p class="tw-text-3xl tw-font-bold tw-mt-2">
                    Rp {{ totalSaldo.toLocaleString("id-ID") }}
                </p>
                <p class="tw-text-sm tw-opacity-80 tw-mt-1">
                    Update: {{ lastUpdate }}
                </p>
            </div>
            ...
            <div class="tw-grid tw-grid-cols-1 tw-gap-4 tw-w-full">
                <!-- Simpanan Pokok -->
                <div
                    class="tw-bg-white tw-rounded-xl tw-shadow-sm tw-p-4 tw-flex tw-justify-between tw-items-center"
                >
                    <div>
                    ...
                    <div class="tw-text-right">
                        <p class="tw-text-orange-600 tw-font-semibold">
                            Rp
                            {{ props.simpanan.pokok.toLocaleString("id-ID") }}
                        </p>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="tw-text-right">
                        <p class="tw-text-orange-600 tw-font-semibold">
                            Rp
                            {{ props.simpanan.wajib.toLocaleString("id-ID") }}
                        </p>
                    </div>
                </div>
                ...
                >
                    ...
                    <div class="tw-text-right">
                        <p class="tw-text-orange-600 tw-font-semibold">
                            Rp
                            {{
                                props.simpanan.sukarela.toLocaleString("id-ID")
                            }}
                        </p>
                    </div>
                </div>
        ...
        <div class="tw-p-5 tw-flex tw-flex-col tw-items-center tw-gap-5">
            <div class="tw-rounded-2xl tw-w-full tw-p-6 tw-shadow-xl">
                <h1
                    class="tw-text-xl tw-font-bold tw-text-center tw-text-black tw-mb-6 tw-tracking-wide"
                >
        ...
        <div class="tw-p-5 tw-flex tw-flex-col tw-items-center tw-gap-5">
            <div
                class="tw-rounded-2xl tw-w-full tw-text-white tw-p-6 tw-shadow-xl"
            >
                <h1
                    class="tw-text-xl tw-font-bold tw-text-center tw-text-black tw-mb-6 tw-tracking-wide"
                >
                ...
                <van-cell-group>
                    <van-cell
                        ...
                        :value="
                            'Rp ' +
                            Number(
                                props.historynominalsimpanan
                                    .nominalhistorypokok ?? 0,
        ...
        <div>
            ...
            <button
                class="tw-fixed tw-bottom-20 tw-right-6 tw-bg-orange-500 tw-text-white tw-rounded-full tw-w-14 tw-h-14 tw-flex tw-items-center tw-justify-center tw-shadow-lg tw-cursor-pointer tw-transition-transform hover:tw-scale-110 tw-z-50"
                @click="toggleMenu"
            >
                <span
                    class="tw-text-3xl tw-font-bold tw-transition-transform tw-duration-300"
                    >+</span
                >
            </button>
